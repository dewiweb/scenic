...
po::options_description ProgramOptions::createDefaultOptions()
{
    ...
    desc.add_options()
        ("help,h", "produce help")
        ("receiver,r", po::bool_switch(), "run as a receiver")
        ("sender,s", po::bool_switch(), "run as a sender")
        ("address,i", po::value<string>()->default_value("127.0.0.1"), "IP address of remote host, or RTSP URI when in rtsp-server mode")
        ...
        ("audioport,t", po::value<int>()->default_value(5000), "audioport number (1024-65535")
        ("videoport,p", po::value<int>()->default_value(5002), "videoport number (1024-65535)")
        ...
        ("videoquality,X", po::value<int>()->default_value(32),
         "use specified video quality instead of bitrate for theora or h264 (0-63)")
        ...
        ("disable-jack-autoconnect,J", po::bool_switch(), "make sure milhouse's "
         "jack audio ports don't connect to system ports automatically on startup")
        ...
        ("vumeter-id", po::value<unsigned long>()->default_value(0), "xid of the GtkSocket where vumeters should be displayed")
