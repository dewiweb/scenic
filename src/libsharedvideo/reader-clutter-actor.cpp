...
{
    ClutterActor *rect_actor = (ClutterActor *)g_object_get_data (G_OBJECT (timeline), "rect_actor");
    if (rect_actor != NULL)
    ...
    ClutterActor *texture_actor = (ClutterActor *)g_object_get_data (G_OBJECT (timeline), "texture_actor");
...
update_texture_actor (gpointer data)
{
	...
	(GAsyncQueue *)g_object_get_data (G_OBJECT (texture_actor), "queue_input_buf");
    GAsyncQueue *queue_output_buf =
	(GAsyncQueue *)g_object_get_data (G_OBJECT (texture_actor), "queue_output_buf");
    GstGLBuffer *gst_gl_buf = (GstGLBuffer *)g_async_queue_pop (queue_input_buf);
    //ClutterActor *stage = (ClutterActor *)g_object_get_data (G_OBJECT (texture_actor), "stage");
...
on_first_video_data (ScenicSharedVideo::Reader *context, void *user_data)
{
    ...
    clutter_actor_set_position (clutter_texture, 300, 170);
    clutter_actor_set_scale (clutter_texture, 0.6, 0.6);
    clutter_actor_show (clutter_texture);
    g_object_set_data (G_OBJECT (clutter_texture), "stage", stage);
...
/* append a gst-gl texture to this queue when you do not need it no more */
    ...
    g_object_set_data (G_OBJECT (timeline), "texture_actor", clutter_texture);
    ...
    gst_bin_add_many (GST_BIN (pipeline), funnel,glupload, fakesink, NULL);
    gst_element_link_many (funnel, glupload, fakesink, NULL);
...
main (int argc, char *argv[])
{
    ...
    g_object_set_data (G_OBJECT (timeline), "rect_actor", rect_actor);
